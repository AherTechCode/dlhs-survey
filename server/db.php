...
class DbHandlers
{
    ...
    private function do_conn()
    {
        ...
        try {
            $this->conn = new PDO("mysql:host=localhost;dbname=survey", "root", "Tope1234$");
    ...
    public function show_dbTables($databasename)
    {
        $this->do_conn();
        try {
            if ($databasename != '') {
                $stmt = $this->conn->prepare("SHOW TABLES FROM $databasename");
                $stmt->execute();
                $each_rec = $stmt->fetchAll(PDO::FETCH_ASSOC);
